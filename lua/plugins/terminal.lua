...
return {
	{
		...
		config = function()
			require("toggleterm").setup({
				size = 20,
				open_mapping = [[<M-\>]], -- Changed from <c-\> to <M-\> (alt-\)
				hide_numbers = true,
				shade_filetypes = {},
			...
			function _G.set_terminal_keymaps()
				local opts = {buffer = 0}
				-- Exit terminal mode - updated to use alt-\ consistently
				...
				-- Window operations from terminal mode (fix for splitting) - updated to use alt-\
				vim.keymap.set('t', '<C-w>', [[<M-\><C-n><C-w>]], opts)
				vim.keymap.set('t', '<C-w>s', [[<M-\><C-n><C-w>s]], opts)
				vim.keymap.set('t', '<C-w>v', [[<M-\><C-n><C-w>v]], opts)
				vim.keymap.set('t', '<C-w>n', [[<M-\><C-n><C-w>n]], opts)
			end
